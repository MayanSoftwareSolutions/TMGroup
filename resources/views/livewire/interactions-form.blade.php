         ...
         <div class="relative">
            ...
            <h5 class="font-extrabold text-lg tracking-tight text-gray-700 ml-3 border-b border-gray-400">
               ...
               <div class="absolute right-0 block">
                  <a href="{{ route('dashboard') }}" class="inset-y-0 right-0 bg-blue-600 hover:bg-purple-600 text-white font-bold text-xs py-2 px-4 rounded">Back to Dashboard</a>
               </div>
        </div>
      </div>
      <br>
                                        ...
                                        @if ($currentPage === 1)
                                            <div class="flex flex-col">
                                                ...
                                                <div class="w-full py-2">
                                                    <label for="contact_form_id" class="block text-sm font-bold text-gray-700">Online Contact Reference No</label>
                                                    <input wire:model.lazy="contact_form_id" type="number" name="contact_form_id" id="contact_form_id" autocomplete="contact_form_id" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                                    @error('contact_form_id') <span class="text-xs text-red-500 mt-1">{{ $message }}</span> @enderror
                                                </div>
    
                                                <div class="w-full py-2">
                                                    <label for="recipient" class="block text-sm font-bold text-gray-700">Recipient</label>
                                                    <input wire:model.lazy="recipient" type="email" name="recipient" id="recipient" autocomplete="recipient" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                                    @error('recipient') <span class="text-xs text-red-500 mt-1">{{ $message }}</span> @enderror
                                                </div>
    
                                                <div class="w-full py-2">
                                                    <label for="interaction_type" class="block text-sm font-bold text-gray-700">Interaction Type</label>
                                                    
                                                    <select name="interaction_type" wire:model="interaction_type" id="interaction_type" class="form-multiselect block rounded-md shadow-sm mt-1 block w-full text-sm">
                                                        <option value="">**Please select interaction type**</option>
                                                        <option value="Acknowledgement">Acknowledgement</option>
                                                        <option value="Response">Response</option>
                                                        <option value="General">General</option>
                                                    </select>
                                                    @error('interaction_type') <span class="text-xs text-red-500 mt-1">{{ $message }}</span> @enderror
                                                </div>
                                            ...
                                            <div class="flex flex-col">
                                                ...
                                                <div class="w-full py-2">
                                                    ...
                                                    <input wire:model.lazy="subject" type="text" name="subject" id="subject" autocomplete="contact_form_id" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                                    @error('subject') <span class="text-xs text-red-500 mt-1">{{ $message }}</span> @enderror
                                                </div>

                                                <div class="w-full py-2">
                                                    <label for="context" class="block text-sm font-bold text-gray-700">Email Body</label>
                                                    <textarea wire:model.lazy="context" type="text" name="context" id="context" autocomplete="contact_form_id" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">

                                                    </textarea>
                                                    @error('context') <span class="text-xs text-red-500 mt-1">{{ $message }}</span> @enderror
                                                </div>
