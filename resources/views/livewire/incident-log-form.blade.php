...
<div class="max-w-7xl mt-8 mx-auto rounded-lg py-5 sm:px-3 lg:px-4">
   <div class="relative">
      ...
      <div class="absolute right-0 block">
         <a href="{{ route('dashboard') }}" class="inset-y-0 right-0 bg-blue-600 hover:bg-purple-600 text-white font-bold text-xs py-2 px-4 rounded">Back to Accounts</a>
      </div>
   </div>
</div>
<br>
                     ...
                     <div class="px-4 py-5 bg-white sm:p-6">
                        ...
                        <div class="flex flex-col">
                           <div class="w-full py-2">
                              <label for="log_reference" class="block text-sm font-bold text-gray-700">Your Reference</label>
                              <input wire:model.lazy="log_reference" type="text" name="log_reference" value="{{ $log_reference }}" id="log_reference" autocomplete="family-name" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 bg-gray-100 rounded-md" readonly>
                              @error('log_reference') <span class="text-xs text-red-500 mt-1">{{ $message }}</span> @enderror
                           </div>
                           <div class="w-full py-2">
                              <label for="user" class="block text-sm font-bold text-gray-700">Your Name</label>
                              <input type="text" value="{{ $user->name }}"class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md bg-gray-100" readonly>
                              @error('user') <span class="text-xs text-red-500 mt-1">{{ $message }}</span> @enderror
                           </div>
                           <div class="w-full py-2">
                              <label for="organisation" class="block text-sm font-bold text-gray-700">Your Organisation</label>
                              <input type="text" autocomplete="organisation" value="{{ $user->organisation }}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full bg-gray-100 shadow-sm sm:text-sm border-gray-300 rounded-md" readonly>
                        ...
                        <div class="flex flex-col">
                           <div class="w-full py-2">
                              <label for="incident_detail" class="block text-sm font-bold text-gray-700" autocomplete="off">Details of incident</label>
                              <textarea wire:model.lazy="incident_detail" type="text" name="incident_detail" id="incident_detail" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                              @error('incident_detail') <span class="text-xs text-red-500 mt-1">{{ $message }}</span> @enderror
                           </div>
                        </div>
                        @endif
                     </div>
